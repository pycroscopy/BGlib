...
class SidpyBandExcitationProcessor(QMainWindow):
    ...
    def load_raw_data(self):
        ...
        if filename:
            try:
                ...
                self.beps_raw = reader.read()[:2,:2,:,:,:1,:1]#TODO: temporary for testing only!!
    ...
    # tab (QWidget)
    # │
    # └── main_layout (QHBoxLayout)      ← main horizontal layout dividing the tab into Left (controls) and Right (output)
    #     │
    #     ├── controls_layout (QVBoxLayout)     ← Left side (stacked vertically)
    #     │   │
    #     │   ├── fit_frame (QFrame, StyledPanel)
    #     │   │   └── fit_layout (QGridLayout)
    #     │   │       ├── QLabel("<b>Fitting Parameters</b>")
    #     │   │       ├── QLabel("Lower Bound")
    #     │   │       ├── QLabel("Upper Bound")
    #     │   │       ├── for each parameter (Amplitude, Frequency, Q, Phase):
    #     │   │       │     ├── QLabel(label)
    #     │   │       │     ├── QLineEdit(lower_bound_field)
    #     │   │       │     └── QLineEdit(upper_bound_field)
    #     │   │       └── (All widgets placed in a grid, 3 columns: Label | Lower | Upper)
    #     │   │
    #     │   ├── cluster_frame (QFrame, StyledPanel)
    #     │   │   └── cluster_layout (QGridLayout)
    #     │   │       ├── QLabel("<b>Clustering Parameters</b>")
    #     │   │       ├── QLabel("Number of Clusters") + QLineEdit("2")
    #     │   │       ├── QLabel("Number of Workers")  + QLineEdit("4")
    #     │   │       ├── QLabel("K-means Guess") + QComboBox(["Yes", "No"])
    #     │   │       ├── QPushButton("Do Guess")
    #     │   │       └── QPushButton("Do Fit")
    #     │   │
    #     │   └── controls_layout.addStretch()   ← pushes content upward nicely
    #     │
    #     └── right_layout (QVBoxLayout)         ← Right side (vertical layout)
    #         │
    #         └── output_box (QTextEdit)         ← Displays print/log output
    ...
    def on_do_guess(self):
        ...
        # print(num_clusters, num_workers, kmeans_guess)
        ...
        # print("Guess shape:", self.fitter.prior.shape)
        # print("Example guess for first spectrum:", self.fitter.prior[0:5])        
    ...
    def on_do_loop_guess(self):
        try:
            ...
            self.loop_output_box.append(f'{loop_num_clusters}')
            self.loop_output_box.append(f'{loop_num_workers}')
            self.loop_output_box.append(f'{loop_kmeans_guess}')
            self.loop_output_box.append(f'{loop_guess_fn}')
            self.loop_output_box.append(f'{d_guess_fn[loop_guess_fn]}')
            self.loop_output_box.append(f'{loop_ind_dims}')
