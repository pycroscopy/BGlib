...
class SidpyBandExcitationProcessor(QMainWindow):
    ...
    def create_fit_loops_tab(self):
        ...
        self.loop_map_fig = Figure(figsize=(4, 3))
        self.loop_map_canvas = FigureCanvas(self.loop_map_fig)
        self.loop_map_ax = self.loop_map_fig.add_subplot(111)
        self.loop_map_im = None
        left_container.addWidget(QLabel("Click a pixel to select a dataset"))
        ...
        # Dropdowns for Cycle and Field
        controls_row = QHBoxLayout()
        controls_row.addWidget(QLabel("Cycle:"))
        self.loop_cycle_combo = QComboBox()
        controls_row.addWidget(self.loop_cycle_combo)

        controls_row.addWidget(QLabel("Field:"))
        self.loop_field_combo = QComboBox()
        controls_row.addWidget(self.loop_field_combo)
        right_container.addLayout(controls_row)

        # Plot Canvas
        self.loop_plot_fig = Figure(figsize=(5, 3))
        self.loop_plot_canvas = FigureCanvas(self.loop_plot_fig)
        self.loop_plot_ax = self.loop_plot_fig.add_subplot(111)
        self.loop_plot_ax.set_xlabel("Voltage (V)")
        self.loop_plot_ax.set_ylabel("Piezoresponse (arb.)")
        right_container.addWidget(self.loop_plot_canvas)
