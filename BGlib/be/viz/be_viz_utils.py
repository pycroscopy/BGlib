...
from sidpy.viz.plot_utils import (
    plot_curves,
    plot_map_stack,
    get_cmap_object,
    plot_map,
...
def visualize_sho_results(
    ...
    def __plot_loops_maps(
        ...
        fig, ax = plot_curves(
            ...
            evenly_spaced=True,
            num_plots=25,
            x_label=spec_var_title,
            y_label=meas_var_title,
            subtitle_prefix="Position",
            title=plt_title,
        )
        if save_plots:
            fig.savefig(
                os.path.join(folder_path, basename + "_" + plt_title + ".png"),
                format="png",
                dpi=300,
            )

        plt_title = grp_name + "_" + win_title + "_Snaps"
        fig, axes = plot_map_stack(
            resp_mat.reshape(num_rows, num_cols, resp_mat.shape[1]),
            color_bar_mode="each",
            evenly_spaced=True,
            subtitle="UDVS Step #",
            title=plt_title,
    ...
    if meas_type is None and field_mode is None:
        ...
        grp_name = "_".join(
            [grp_name, sho_grp.name.split("/")[-1].split("-")[0], h5_main.name.split("/")[-1]]
        )
    ...
    if isBEPS:
        ...
        if meas_type == "AC modulation mode with time reversal":
            ...
            if field_mode == "in and out-of-field":
                ...
                for win_title, resp_mat in zip(
                    ["In_Field", "Out_of_Field"], [in_phase * in_amp, out_phase * out_amp]
                ):
                    fig_list.append(
                        __plot_loops_maps(
                            dc_vec,
        ...
        fig_ms, ax_ms = plot_map_stack(
            np.dstack((amp_mat, freq_mat, q_mat, phase_mat, rsqr_mat)).T,
            num_comps=5,
            color_bar_mode="each",
            title=grp_name,
            subtitle=[
...
def plot_loop_guess_fit(vdc, ds_proj_loops, ds_guess, ds_fit, title=""):
    ...
    for ax, pos in zip(axes.flat, positions):
        ...
        ax.plot(
            vdc_shifted,
            loop_fit_function(vdc_shifted, np.array(list(ds_guess[pos]))),
            "g",
            label="guess",
        )
        ax.plot(
            vdc_shifted,
            loop_fit_function(vdc_shifted, np.array(list(ds_fit[pos]))),
            "r--",
            label="Fit",
        )
...
def jupyter_visualize_beps_sho(
    pc_sho_dset, step_chan, resp_func=None, resp_label="Response", cmap=None, verbose=False
):
    """
    ...
    bias_mat = np.transpose(bias_mat[spec_step_dim_ind], new_spec_order).reshape(
        sho_dset_collapsed.shape[len(pos_dims) :]
    )
    if bias_mat.ndim == 1:
    ...
    def dc_spectroscopy_func(resp_vec):
        return resp_vec["Amplitude [V]"] * np.cos(resp_vec["Phase [rad]"]) * 1e3
    ...
    resp_dict = {pos_labels[-1]: row_ind, pos_labels[-2]: col_ind}
    ...
    spatial_map = pc_sho_dset.slice(spatial_dict, as_scalar=False, verbose=verbose)[0][
        sho_quantity
    ].squeeze()
    resp_vec = resp_func(
        pc_sho_dset.slice(resp_dict, as_scalar=False, verbose=verbose)[0].reshape(bias_mat.shape)
    )
    ...
    for ikey, key in enumerate(pos_labels[:-2]):
        pos_sliders[key] = widgets.IntSlider(
            value=0,
            min=0,
            max=pos_dims[ikey] - 1,
            step=1,
            description="{} Step:".format(key),
    ...
    def update_resp_plot(resp_dict):
        ...
        resp_vec = resp_func(
            pc_sho_dset.slice(resp_dict, as_scalar=False, verbose=verbose)[0].reshape(
                bias_mat.shape
            )
        ).T
    ...
    sho_quantity_picker = widgets.Dropdown(
        options=list(sho_dset_collapsed.dtype.names[:-1]), description="SHO Quantity"
    )
    bias_step_picker = widgets.IntSlider(
        min=0, max=bias_mat.shape[0] - 1, step=1, description="Bias Step"
    )
...
def jupyter_visualize_be_spectrograms(pc_main, cmap=None):
    ...
    num_udvs_steps = int(
        np.prod([spec_dims[idim] for idim in range(len(spec_dims)) if idim != ifreq])
    )
    ...
    if len(pos_dims) >= 2:
        ...
        pos_heading = (
            pos_labels[-1] + ": " + str(xdata) + ", " + pos_labels[-2] + ": " + str(ydata) + ", "
        )
        ...
        spec_index_slider = widgets.IntSlider(
            value=0, min=0, max=pc_main.shape[1], step=1, description="Step"
        )
        ...
        def plot_spectrogram(data, freq_vals):
            ...
            im_handles.append(
                axes[0].imshow(
                    np.abs(data),
                    cmap=cmap,
                    extent=[freqs_2d[0, 0], freqs_2d[-1, 0], data.shape[0], 0],
                    interpolation="none",
                )
            )
        ...
        fig, axes, im_handles = plot_spectrogram(
            np.reshape(pc_main[0], (num_udvs_steps, -1)), freqs_2d
        )
...
def jupyter_visualize_beps_loops(
    h5_projected_loops, h5_loop_guess, h5_loop_fit, step_chan="DC_Offset", cmap=None
):
    """
    ...
    h5_proj_spec_inds = get_auxiliary_datasets(
        h5_projected_loops, aux_dset_name="Spectroscopic_Indices"
    )[-1]
    h5_proj_spec_vals = get_auxiliary_datasets(
        h5_projected_loops, aux_dset_name="Spectroscopic_Values"
    )[-1]
    h5_pos_inds = get_auxiliary_datasets(h5_projected_loops, aux_dset_name="Position_Indices")[-1]
    ...
    new_order = (
        list(range(len(pos_dims))) + [len(pos_dims) + spec_step_dim_ind] + rest_loop_dim_order
    )
...
def jupyter_visualize_parameter_maps(h5_loop_parameters, cmap=None, **kwargs):
    ...
    parameter_map_stack = np.reshape(
        h5_loop_parameters[parameter_names[0]], [pos_dims[0], pos_dims[1], -1]
    )
    ...
    fig, axes = plot_map_stack(
        parameter_map_stack,
        num_comps=num_cycles,
        color_bar_mode="each",
        subtitle=map_titles,
        title="Maps of Loop Parameter {}".format(parameter_names[0]),
        **kwargs,
    )

    def update_loop_maps(parameter_name):
        parameter_map_stack = np.reshape(
            h5_loop_parameters[parameter_name], [pos_dims[0], pos_dims[1], -1]
        )
        parameter_map_stack = np.moveaxis(parameter_map_stack, -1, 0)
        ...
        for icycle, ax_cycle in enumerate(axes[:num_cycles]):
            ...
            image.set_clim(
                vmin=np.min(parameter_map_stack[icycle, :, :]),
...
def jupyter_visualize_loop_sho_raw_comparison(h5_loop_parameters, cmap=None):
    ...
    loop_bias_vec = np.moveaxis(loop_bias_vec, sho_bias_dim, 0).reshape(
        sho_spec_dims[sho_bias_dim], -1
    )
    ...
    def _get_loop_map(selected_loop_parm, selected_loop_cycle):
        ...
        loop_parameter_map = np.reshape(
            h5_loop_parameters[selected_loop_parm, :, selected_loop_cycle],
        ...
        for label in loop_parameter_spec_labs:
            val = h5_loop_spec_vals[get_attr(h5_loop_spec_vals, label)].squeeze()[
                selected_loop_cycle
            ]
    ...
    def _get_loops(selected_loop_cycle, selected_loop_pos):
        ...
        loop_guess_vec = loop_fit_function(
            selected_loop_bias_vec, h5_loop_guess[pos_ind, selected_loop_cycle].tolist()
        )
        loop_fit_vec = loop_fit_function(
            selected_loop_bias_vec, h5_loop_fit[pos_ind, selected_loop_cycle].tolist()
        )
    ...
    def _get_sho(selected_loop_pos, selected_step, selected_loop_cycle):
        ...
        w_vec = np.moveaxis(full_w_vec, main_bias_dim - len(pos_dims), -1)[selected_step][
            selected_loop_ndims
        ]
        ...
        raw_data_vec, _ = reshape_to_n_dims(
            np.atleast_2d(h5_main[pos_ind]), h5_spec=h5_main_spec_inds
        )
        raw_data_vec = np.moveaxis(raw_data_vec.squeeze(), main_bias_dim - len(pos_dims), -1)[
            selected_step
        ][selected_loop_ndims]
    ...
    current_w_vec, current_sho_guess, current_sho_fit, current_raw_data = _get_sho(
        selected_loop_pos, selected_step, selected_loop_cycle
    )
    ...
    # Plot the Loops
    plt_loop_proj = ax_loop_pgf.plot(
        current_loop_bias_vec, current_loop_proj_vec, "k", label="Loop Projection"
    )
    plt_loop_guess = ax_loop_pgf.plot(
        current_loop_bias_vec, current_loop_guess_vec, "g", label="Loop Guess"
    )
    plt_loop_fit = ax_loop_pgf.plot(
        current_loop_bias_vec, current_loop_fit_vec, "r--", label="Loop Fit"
    )
    ax_loop_pgf.legend()

    # Plot the amplitude of the SHO Guess, SHO Fit, and Raw Data
    plt_raw_data_amp = ax_sho_amp.plot(
        current_w_vec, np.abs(current_raw_data), "k", label="Raw Data"
    )
    plt_sho_guess_amp = ax_sho_amp.plot(
        current_w_vec, np.abs(current_sho_guess), "g", label="SHO Guess"
    )
    plt_sho_fit_amp = ax_sho_amp.plot(
        current_w_vec, np.abs(current_sho_fit), "r--", label="SHO Fit"
    )
    ax_sho_amp.legend()

    # Plot the phase of the SHO Guess, SHO Fit, and Raw Data
    plt_raw_data_phase = ax_sho_phase.plot(
        current_w_vec, np.angle(current_raw_data), "k", label="Raw Data"
    )
    plt_sho_guess_phase = ax_sho_phase.plot(
        current_w_vec, np.angle(current_sho_guess), "g", label="SHO Guess"
    )
    plt_sho_fit_phase = ax_sho_phase.plot(
        current_w_vec, np.angle(current_sho_fit), "r--", label="SHO Fit"
    )
    ax_sho_phase.legend()
    ...
    def _update_loop_plots(selected_loop_cycle, x_pos, y_pos):
        (
            current_loop_bias_vec,
            current_loop_proj_vec,
            current_loop_guess_vec,
            current_loop_fit_vec,
    ...
    def _update_sho_plots(selected_loop_cycle, x_pos, y_pos, selected_step):
        current_w_vec, current_sho_guess, current_sho_fit, current_raw_data = _get_sho(
            [x_pos, y_pos], selected_step, selected_loop_cycle
        )
    ...
    display(save_fig_filebox_button(fig, fig_filename + ".png"))
    display(fig)
    x_pos_widget = widgets.FloatSlider(
        min=0.0, max=float(pos_dims[0]), step=pos_dims[0] / 100, value=selected_loop_pos[0]
    )
    y_pos_widget = widgets.FloatSlider(
        min=0.0, max=float(pos_dims[1]), step=pos_dims[1] / 100, value=selected_loop_pos[1]
    )
    loop_cycle_widget = widgets.IntSlider(
        min=0, max=loop_num_cycles, step=1, value=selected_loop_cycle
    )
    ...
    widgets.interact(_update_crosshairs, x_pos=x_pos_widget, y_pos=y_pos_widget)
    widgets.interact(
        _update_loop_plots,
...
def plot_loop_sho_raw_comparison(
    h5_loop_parameters,
    h5_sho_grp,
    h5_raw_dset,
    ...
    def _get_loops(selected_loop_cycle, selected_loop_pos):
        ...
        loop_guess_vec = loop_fit_function(
            selected_loop_bias_vec, loop_guess_slice.squeeze().tolist()
        )
        loop_fit_vec = loop_fit_function(selected_loop_bias_vec, loop_fit_slice.squeeze().tolist())
    ...
    def _get_sho(selected_loop_pos, selected_step, selected_loop_cycle):
        ...
        # Get the frequency vector for the selected step
        w_vec = np.moveaxis(
            full_w_vec, main_freq_dim, len(main_spec_dims) - 1
    ...
    # Plot the Loops
    loop_proj_plot = ax_loop_pgf.plot(
        current_loop_bias_vec, current_loop_proj_vec, "k", label="Loop Projection"
    )[0]
    loop_guess_plot = ax_loop_pgf.plot(
        current_loop_bias_vec, current_loop_guess_vec, "g", label="Loop Guess"
    )[0]
    loop_fit_plot = ax_loop_pgf.plot(
        current_loop_bias_vec, current_loop_fit_vec, "r--", label="Loop Fit"
    )[0]
    ax_loop_pgf.legend()
    loop_pgf_title = " - ".join(
        ["Position: ({}, {})".format(selected_loop_pos[0], selected_loop_pos[1]), loop_map_title]
    )
    ...
    # Plot the amplitude of the SHO Guess, SHO Fit, and Raw Data
    raw_amp_plot = ax_sho_amp.plot(
        current_w_vec, np.abs(current_raw_data), "k*", label="Raw Data Amplitude"
    )[0]
    guess_amp_plot = ax_sho_amp.plot(
        current_w_vec, np.abs(current_sho_guess), "g", label="SHO Guess Amplitude"
    )[0]
    fit_amp_plot = ax_sho_amp.plot(
        current_w_vec, np.abs(current_sho_fit), "r:", label="SHO Fit Amplitude"
    )[0]
    ax_sho_amp.legend()
    ...
    # Plot the phase of the SHO Guess, SHO Fit, and Raw Data
    raw_phase_plot = ax_sho_phase.plot(
        current_w_vec, np.angle(current_raw_data), "k*", label="Raw Data Phase"
    )[0]
    guess_phase_plot = ax_sho_phase.plot(
        current_w_vec, np.angle(current_sho_guess), "g", label="SHO Guess Phase"
    )[0]
    fit_phase_plot = ax_sho_phase.plot(
        current_w_vec, np.angle(current_sho_fit), "r:", label="SHO Fit Phase"
    )[0]
    ax_sho_phase.legend()
    ...
    loop_cycle_widget = widgets.IntSlider(
        min=0, max=loop_num_cycles - 1, step=1, value=selected_loop_cycle
    )
    ...
    def _update_loop_plots(selected_loop_cycle, xdata, ydata):
        ...
        loop_pgf_title = " - ".join(["Position: ({}, {})".format(xdata, ydata), loop_map_title])
        ax_loop_pgf.set_title(loop_pgf_title)
    ...
    def _update_sho_plots(selected_loop_cycle, selected_step, xdata, ydata):
        ...
        current_w_vec, current_sho_guess, current_sho_fit, current_raw_data = _get_sho(
            (xdata, ydata), selected_step, selected_loop_cycle
        )
        ...
        ax_sho_amp.set_ylim(
            [
                np.min(np.abs([current_raw_data, current_sho_guess, current_sho_fit])) * 0.95,
...
def _add_loop_parameters(axes, switching_coef_vec):
    ...
    for ax, pos in zip(axes.flat, positions):
        ...
        ax.axvline(
            switching_coef_vec[pos]["Nucleation Bias 1"], c="k", ls=":", label="Nucleation Bias 1"
        )
        ax.axvline(
            switching_coef_vec[pos]["Nucleation Bias 2"], c="r", ls=":", label="Nucleation Bias 2"
        )
...
def plot_1d_spectrum(data_vec, freq, title, **kwargs):
    ...
    if len(data_vec) != len(freq):
        raise ValueError(
            "Incompatible data sizes! spectrum: "
            + str(len(data_vec))
            + ", frequency: "
            + str(freq.shape)
        )
    ...
    fig, axes = plot_complex_spectra(
        np.expand_dims(data_vec, axis=0),
        freq,
        title=title,
        subtitle_prefix="",
        num_comps=1,
...
def plot_2d_spectrogram(mean_spectrogram, freq, title=None, **kwargs):
    ...
    if mean_spectrogram.shape[1] != freq.size:
        if mean_spectrogram.shape[0] == freq.size:
            ...
            raise ValueError(
                "plot_2d_spectrogram: Incompatible data sizes!!!! spectrogram: "
                + str(mean_spectrogram.shape)
                + ", frequency: "
                + str(freq.shape)
            )

    fig, axes = plot_complex_spectra(
        np.expand_dims(mean_spectrogram, axis=0),
        num_comps=1,
        title=title,
        x_label="Frequency (kHz)",
        y_label="UDVS step",
...
def viz_berelaxfit(
    ...
    mixed_signal = (
        sho_fit["Amplitude [V]"] * sensitivity * np.cos(sho_fit["Phase [rad]"] + phase_offset)
    )
    ...
    if starts_with == "write":
        ...
        if isinstance(write_dc_offset_values, float):
            write_dc_offset_values = [write_dc_offset_values]

    if starts_with == "read":
        for i in range(no_rs_spectra):
            read_inds_split.append(all_inds_split[i][: -int(no_write_steps)])
            write_dc_offset_values = sho_fit.h5_spec_vals[
                1, np.argwhere(sho_fit.h5_spec_vals[0] == no_read_steps)
            ]
        if isinstance(write_dc_offset_values, float):
            write_dc_offset_values = [write_dc_offset_values]
    ...
    if t_time in read_inds_split[bias_ind]:
        ...
        plt.plot(time_axis, mixed_signal_reshape[x_col, h_row, read_inds_split[bias_ind]], "ro")
    ...
    if t_time not in read_inds_split[bias_ind]:
        ...
        plt.plot(time_axis, mixed_signal_reshape[x_col, h_row, read_inds_split[bias_ind]], "ro")
    plt.title(
        "Response at ({},{}) after {} V".format(x_col, h_row, write_dc_offset_values[bias_ind])
    )
    ...
    params = (
        sho_amplitude[x_col, h_row, t_time],
        sho_frequency[x_col, h_row, t_time],
        sho_quality_factor[x_col, h_row, t_time],
        sho_phase[x_col, h_row, t_time],
    )
    ...
    plt.axvspan(
        xmin=read_inds_split[bias_ind].min(), xmax=read_inds_split[bias_ind].max() + 1, alpha=0.1
    )
    ...
    plt.subplot(423)
    plt.title(
        "SHO Fitted Phase ({} V)".format(write_dc_offset_values[bias_ind])
    )  # after applying {} V'.format(write_dc_offset_values[bias_ind]))
    plt.plot(
        time_axis,
        sho_fit["Phase [rad]"].reshape(x_length, y_length, -1)[
            x_col, h_row, read_inds_split[bias_ind]
        ],
        "ro",
    )
    plt.xlabel("Time Elapsed (s)")
    ...
    if fit_method == "Exponential" or "Double_Exp" or "Str_Exp":
        plt.subplot(425)
        plt.title(
            "SHO Fitted AMPLITUDE ({} V)".format(write_dc_offset_values[bias_ind])
        )  # after applying {} V'.format(write_dc_offset_values[bias_ind]))
        plt.plot(
            time_axis,
            sho_fit["Amplitude [V]"].reshape(x_length, y_length, -1)[
                x_col, h_row, read_inds_split[bias_ind]
            ],
            "go",
        )
