...
class BEodfTranslator(Translator):
    ...
    def is_valid_file(data_path):
        ...
        parm_found = any([piece in path_dict.keys() for piece in ["parm_txt", "old_mat_parms"]])
        real_found = any([piece in path_dict.keys() for piece in ["read_real", "write_real"]])
        imag_found = any([piece in path_dict.keys() for piece in ["read_imag", "write_imag"]])
    ...
    def translate(
        self, file_path, show_plots=True, save_plots=True, do_histogram=False, verbose=False
    ):
        """
        ...
        if "parm_txt" in path_dict.keys():
            if self._verbose:
                logger.info("\treading parameters from text file")
            isBEPS, parm_dict = parmsToDict(path_dict["parm_txt"])

        elif "old_mat_parms" in path_dict.keys():
            if self._verbose:
                logger.info("\treading parameters from old mat file")
            parm_dict = self._get_parms_from_old_mat(
                path_dict["old_mat_parms"], verbose=self._verbose
            )
        ...
        for case in [
            ("VS_mode", "AC modulation mode", "AC modulation mode with time reversal"),
            (
                "VS_mode",
                "load Arbitrary VS Wave from text file",
            ...
            if parm_dict[key] == wrong_val:
                warn(
                    'Updating parameter "{}" from invalid value of "{}" to "{}"'.format(
                        key, wrong_val, corr_val
                    )
                )
                parm_dict[key] = corr_val

        # Some .mat files did not set correct values to some parameters:
        for case in [("BE_amplitude_[V]", 1e-2, 0.5151), ("VS_amplitude_[V]", 1e-2, 0.9876)]:
        ...
        if self._verbose:
            ...
            for key in keys:
                logger.info("\t\t{} : {}".format(key, parm_dict[key]))

            logger.info("\n\tisBEPS = {}".format(isBEPS))

        if parm_dict["BE_bins_per_band"] > 300:
            raise ValueError(
                "Too many BE bins per band: {}. Translation would"
        ...
        # Check file sizes:
        if self._verbose:
            logger.info("\tChecking sizes of real and imaginary data files")
        ...
        if self._verbose:
            logger.info("\tLooking for secondary channels")
        ...
        if self._verbose:
            logger.info("\tRows: {}, Cols: {}".format(num_rows, num_cols))
        ...
        if self._verbose:
            logger.info("\tChecking bins: Total: {}, actual: {}".format(tot_bins, check_bins))
        ...
        if tot_bins % 1 and check_bins % 1:
            ...
            tot_bins = check_bins
            warn(
                "Warning:  A pixel seems to be missing from the data. "
        ...
        if isBEPS and tot_bins % 1 == 0 and parm_dict["VS_mode"] != "Custom":
            bins_per_step = (
                parm_dict["FORC_num_of_FORC_cycles"]
            ...
            if verbose:
                logger.info(
                    "\t\tNumber of bins per step: calculated: {}, actual {}".format(
            ...
            if bins_per_step > 0:
                ...
                if verbose:
                    logger.info(
                        "\t\tUnable to calculate number of bins per step "
        ...
        if isBEPS:
            if self._verbose:
                logger.info("\tBuilding UDVS table for BEPS")
            UDVS_labs, UDVS_units, UDVS_mat = self._build_udvs_table(parm_dict)

            if self._verbose:
                logger.info("\tTrimming UDVS table to remove unused plot group columns")

            UDVS_mat, UDVS_labs, UDVS_units = trimUDVS(
                UDVS_mat, UDVS_labs, UDVS_units, ignored_plt_grps
            )
            ...
            if self._verbose:
                logger.info(
                    "\t# UDVS steps: {}, # bins/step: {}".format(
                        num_actual_udvs_steps, bins_per_step
                    )
                )

            if bins_per_step % 1:
                logger.info(
                    "UDVS mat shape: {}, total bins: {}, bins per step: {}".format(
                        UDVS_mat.shape, tot_bins, bins_per_step
                    )
                )
            ...
            for step_index in range(UDVS_mat.shape[0]):
                if UDVS_mat[step_index, 2] < 1e-3:  # invalid AC amplitude
                    continue
                # Bin step
                old_spec_inds[0, stind : stind + bins_per_step] = np.arange(
                    bins_per_step, dtype=INDICES_DTYPE
                )
                # UDVS step
                old_spec_inds[1, stind : stind + bins_per_step] = step_index * np.ones(
                    bins_per_step, dtype=INDICES_DTYPE
                )
                stind += bins_per_step
            ...
            if self._verbose:
                logger.info("\tPreparing supporting variables since BE-Line")
            self.signal_type = 1
            ...
            UDVS_units = ["", "V", "A", "", "", ""]
            UDVS_mat = np.array(
                [1, 0, parm_dict["BE_amplitude_[V]"], 1, 1, 1], dtype=float
            ).reshape(1, len(UDVS_labs))

            old_spec_inds = np.vstack(
                (np.arange(tot_bins, dtype=INDICES_DTYPE), np.zeros(tot_bins, dtype=INDICES_DTYPE))
            )

        if "parm_mat" in path_dict.keys():
            if self._verbose:
                logger.info("\treading BE arrays from parameters text file")
            bin_inds, bin_freqs, bin_FFT, ex_wfm = self._read_parms_mat(
                path_dict["parm_mat"], isBEPS
            )
        elif "old_mat_parms" in path_dict.keys():
            if self._verbose:
                logger.info("\treading BE arrays from old mat text file")
            bin_inds, bin_freqs, bin_FFT, ex_wfm, dc_amp_vec = self._read_old_mat_be_vecs(
                path_dict["old_mat_parms"], verbose=verbose
            )
        else:
            warn("No secondary parameters file (.mat) provided. Generating dummy BE arrays")
            ...
            if verbose:
                logger.info("\tGenerating BE arrays of length: {}".format(bins_per_step))
        ...
        if self._verbose:
            logger.info("\tPreparing UDVS slices for region references")
        udvs_slices = dict()
        ...
        if self.expt_type == 2:
            if self._verbose:
                logger.info("\tExperiment type = 2. Doubling BE vectors")
        ...
        if self._verbose:
            logger.info("\tCalculating spectroscopic values")
        ret_vals = createSpecVals(
            UDVS_mat,
            old_spec_inds,
            bin_freqs,
            exec_bin_vec,
        ...
        if self._verbose:
            logger.info("\t\tspec_vals_labs: {}".format(spec_vals_labs))
            unit_vals = get_unit_values(
                spec_inds, spec_vals, all_dim_names=spec_vals_labs, is_spec=True, verbose=False
            )
            logger.info("\tUnit spectroscopic values")
            for key, val in unit_vals.items():
                logger.info("\t\t{} : length: {}, values:\n\t\t\t{}".format(key, len(val), val))

        if spec_inds.shape[1] != tot_bins:
            raise ValueError(
                "Second axis of spectroscopic indices: {} not "
        ...
        if path.exists(h5_path):
            if self._verbose:
                logger.info("\tRemoving existing / old translated file: " + h5_path)
            remove(h5_path)
        ...
        if self._verbose:
            logger.info("\tWriting attributes to HDF5 file root")
        ...
        if self._verbose:
            logger.info("\twriting attributes to Measurement group")
        ...
        write_simple_attrs(
            h5_chan_grp, {"Channel_Input": "IO_Analog_Input_1", "channel_type": "BE"}
        )

        # Now the datasets!
        if self._verbose:
            logger.info("\tCreating ancillary datasets")
        ...
        h5_chan_grp.create_dataset(
            "Bin_Step", data=np.arange(bins_per_step, dtype=INDICES_DTYPE), dtype=INDICES_DTYPE
        )
        ...
        if self._verbose:
            logger.info("\tWriting Position datasets")

        pos_dims = [
            Dimension("X", "m", np.arange(num_cols)),
            Dimension("Y", "m", np.arange(num_rows)),
        ]
        h5_pos_ind, h5_pos_val = write_ind_val_dsets(
            h5_chan_grp, pos_dims, is_spectral=False, verbose=self._verbose
        )
        if self._verbose:
            logger.info("\tPosition datasets of shape: {}".format(h5_pos_ind.shape))

        if self._verbose:
            logger.info("\tWriting Spectroscopic datasets of shape: {}".format(spec_inds.shape))
        h5_spec_inds = h5_chan_grp.create_dataset(
            "Spectroscopic_Indices", data=spec_inds, dtype=INDICES_DTYPE
        )
        h5_spec_vals = h5_chan_grp.create_dataset(
            "Spectroscopic_Values", data=np.array(spec_vals), dtype=VALUES_DTYPE
        )
        for dset in [h5_spec_inds, h5_spec_vals]:
            # TODO: This is completely unnecessary
            write_region_references(
                dset, spec_vals_slices, add_labels_attr=True, verbose=self._verbose
            )
        ...
        if self._verbose:
            logger.info("\tWriting noise floor dataset")
        ...
        """
        BEPS_chunks = calc_chunks(
            [num_pix, tot_bins], np.complex64(0).itemsize, unit_chunks=(1, bins_per_step)
        )
        if self._verbose:
            logger.info("\tHDF5 dataset will have chunks of size: {}".format(BEPS_chunks))
            logger.info(
                "\tCreating empty main dataset of shape: ({}, {})".format(num_pix, tot_bins)
            )
        self.h5_raw = write_main_dataset(
            h5_chan_grp,
            (num_pix, tot_bins),
            "Raw_Data",
            "Piezoresponse",
        ...
        if self._verbose:
            logger.info("\tReading data from binary data files into raw HDF5")
        self._read_data(parm_dict, path_dict, real_size, isBEPS, add_pix)

        if self._verbose:
            logger.info("\tGenerating plot groups")
        generatePlotGroups(
            self.h5_raw,
            self.mean_resp,
            folder_path,
        ...
        if self._verbose:
            logger.info("\tUpgrading to USIDataset")
        ...
        if current_data_exists:  # If a .dat file matches
            if self._verbose:
                logger.info("\tReading data in secondary channels (current)")
    ...
    def _read_data(self, parm_dict, path_dict, real_size, isBEPS, add_pix):
        ...
        if not isBEPS:
            ...
            if self._verbose:
                logger.info("\t\tReading all raw data for BE-Line in one shot")
            self._quick_read_data(
                path_dict["read_real"], path_dict["read_imag"], parm_dict["num_udvs_steps"]
            )
            ...
            if self._verbose:
                logger.info("\t\tReading all raw BEPS (out-of-field) data at once")
            self._quick_read_data(
                path_dict["read_real"], path_dict["read_imag"], parm_dict["num_udvs_steps"]
            )
            ...
            if self._verbose:
                logger.info("\t\tReading all raw BEPS (in-field only) data at once")
            self._quick_read_data(
                path_dict["write_real"], path_dict["write_imag"], parm_dict["num_udvs_steps"]
            )
        else:
            # Large BEPS datasets OR those with in-and-out of field
            if self._verbose:
                logger.info(
                    "\t\tReading all raw data for in-and-out-of-field OR "
                    "very large file one pixel at a time"
                )
            self._read_beps_data(
                path_dict,
    ...
    def _read_beps_data(self, path_dict, udvs_steps, mode, add_pixel=False):
        ...
        logger.info("---- reading pixel-by-pixel ----------")
        ...
        rand_spectra = self._get_random_spectra(
            parsers, self.h5_raw.shape[0], udvs_steps, step_size, num_spectra=self.num_rand_spectra
        )
        ...
        for pix_indx in range(numpix):
            if self.h5_raw.shape[0] > 5:
                if pix_indx % int(round(self.h5_raw.shape[0] / 10)) == 0:
                    logger.info(
                        "Reading... {} complete".format(
                            round(100 * pix_indx / self.h5_raw.shape[0])
                        )
                    )
    ...
    def _quick_read_data(self, real_path, imag_path, udvs_steps):
        ...
        if self._verbose:
            logger.info("\t\t\tChecking if conjugate is required")
        take_conjugate = requires_conjugate(rand_spectra, cores=self._cores)
        raw_vec = parser.read_all_data()
        if take_conjugate:
            if self._verbose:
                logger.info("\t" * 4 + "Taking conjugate for positive quality factors")
            raw_vec = np.conjugate(raw_vec)

        if raw_vec.shape != np.prod(self.h5_raw.shape):
            percentage_padded = (
                100 * (np.prod(self.h5_raw.shape) - raw_vec.shape) / np.prod(self.h5_raw.shape)
            )
            warn(
                "Warning! Raw data length {} is not matching placeholder length {}. "
                "Padding zeros for {}% of the data!".format(
                    raw_vec.shape, np.prod(self.h5_raw.shape), percentage_padded
                )
            )
    ...
    def _read_secondary_channel(self, h5_meas_group, aux_file_path):
        ...
        """
        if self._verbose:
            logger.info("\t---------- Reading Secondary Channel  ----------")
        ...
        if not is_in_out_field and len(aux_file_paths) > 1:
            ...
            warn(
                "\t\tField was not varied but found more than one file for "
                "secondary channel: {}.\n\t\tResults will be overwritten"
                "".format([path.split(item)[-1] for item in aux_file_paths])
            )
        elif is_in_out_field and len(aux_file_paths) == 1:
            warn(
                "\t\tField was varied but only one data file for secondary"
                "channel was found. Half the data will be zeros"
            )

        spectral_len = 1
        for dim_name, dim_size in zip(self.h5_raw.spec_dim_labels, self.h5_raw.spec_dim_sizes):
        ...
        if self._verbose:
            logger.info(
                "\t\tExpecting this channel to be of shape: ({}, {})".format(num_pix, spectral_len)
            )
            logger.info("\t\tis_in_out_field: {}".format(is_in_out_field))
        ...
        write_simple_attrs(
            h5_current_channel_group,
            {"Channel_Input": "IO_Analog_Input_2", "channel_type": "Current"},
            verbose=False,
        )
        ...
        if self._verbose:
            logger.info("\t\tCreated groups, wrote attributes and spec datasets")

        h5_current_main = write_main_dataset(
            h5_current_channel_group,  # parent HDF5 group
            ...
            main_dset_attrs={"IO_rate": 4e6, "Amplifier_Gain": 9},
            verbose=self._verbose,
        )

        if self._verbose:
            logger.info("\t\tCreated empty main dataset:\n{}".format(h5_current_main))

        if is_in_out_field:
            if self._verbose:
                logger.info(
                    "\t\tHalving the spectral length per binary file to: {} "
        ...
        if self._verbose:
            logger.info("\t\tAllowed to read {} pixels per chunk".format(max_pos_per_read))
            logger.info("\t\tStarting to read raw binary data")
        ...
        for aux_file in aux_file_paths:
            ...
            if self._verbose:
                logger.info("\t" * 3 + "Reading file: {}".format(aux_file))
            ...
            while start_pix < num_pix:
                ...
                if self._verbose:
                    logger.info(
                        "\t" * 4
                        + "Reading pixels {} to {} - {} bytes".format(
                            start_pix, end_pix, bytes_to_read
                        )
                    )

                cur_data = np.frombuffer(cur_file.read(bytes_to_read), dtype="f")

                if self._verbose:
                    logger.info("\t" * 4 + "Read vector of shape: {}".format(cur_data.shape))
                    logger.info(
                        "\t" * 4 + "Reshaping to ({}, {})".format(pos_to_read, spectral_len)
                    )
            ...
            if self._verbose:
                logger.info("\t" * 4 + "Done reading binary file")
    ...
    def _read_old_mat_be_vecs(file_path, verbose=False):
        ...
        if verbose:
            for vec, var_name in zip(
                [BE_wave, bin_inds, bin_w, dc_amp_vec_full],
                ["BE_wave", "bin_inds", "bin_w", "dc_amp_vec_full"],
            ):
                logger.info(
                    "\t\t{} has shape: {} and dtype: {}".format(var_name, vec.shape, vec.dtype)
                )
        try:
    ...
    def _get_parms_from_old_mat(file_path, verbose=False):
        ...
        if verbose:
            logger.info("\t\tEstimating File params from path: {}".format(file_path))
        ...
        if ind > 0:
            try:
                ...
                # logger.info('Could not convert "' + suffix + '" to integer')
                pass
        ...
        if verbose:
            logger.info(
                "\t\tEstimating experiment date and time from .mat file header: {} ".format(header)
            )
        targ_str = "Created on: "
        try:
            ...
            dt_obj = datetime.datetime.strptime(header, "%c")
            # '%a %b %d %H:%M:%S %Y')
            # parms.txt contains string formatted as: 09-Apr-2015 HH:MM:SS
            parm_dict["File_date_and_time"] = dt_obj.strftime("%d-%b-%Y %H:%M:%S")
        except ValueError:
            pass

        parm_dict["IO_rate"] = str(int(matread["AO_rate"] / 1e6)) + " MHz"
        ...
        if verbose:
            logger.info("\t\tBE_parm_vec_1: {}".format(BE_parm_vec_1))
            logger.info("\t\tBE_parm_vec_2: {}".format(BE_parm_vec_2))
        ...
        if verbose:
            logger.info("\t\tassembly_parm_vec: {}".format(assembly_parm_vec))
        ...
        if verbose:
            logger.info("\t\tVS_parms: {}".format(VS_parms))
            logger.info("\t\tdc_amp_vec_full: {}".format(dc_amp_vec_full))
        ...
        if VS_parms[0] in [0, 8, 9, 11]:
            if verbose:
                logger.info(
                    "\t\tDC modulation or current mode based on VS_parms[0]: {}".format(VS_parms[0])
                )
            parm_dict["VS_mode"] = "DC modulation mode"
            if VS_parms[0] == 9:
                if verbose:
                    logger.info("\t\tcurrent mode based on VS parms[0]")
                parm_dict["VS_mode"] = "current mode"
            parm_dict["VS_amplitude_[V]"] = 0.5 * (
                max(dc_amp_vec_full) - np.min(dc_amp_vec_full)
            )  # SS_max_offset_amplitude
            parm_dict["VS_offset_[V]"] = np.max(dc_amp_vec_full) + np.min(dc_amp_vec_full)

        elif VS_parms[0] in [1, 6, 7]:
            if verbose:
                logger.info("\t\tFORC, based on VS_parms[0]: {}".format(VS_parms[0]))
            ...
            if True:
                ...
                if verbose:
                    logger.info("\t\t\tGrabbing FORC high and low vals from parms")
            ...
            if verbose:
                logger.info(
                    "\t\tAC Spectroscopy with time reversal, based on VS_parms[0]: {}".format(
                        VS_parms[0]
                    )
                )
            if VS_parms[0] == 3:
            ...
            warn(
                "Unknown VS mode: {}. Assuming user defined custom voltage spectroscopy".format(
                    VS_parms[0]
                )
            )
        ...
        if VS_parms[0] not in [2, 3, 4]:
            if verbose:
                logger.info("\t\tEstimating phase and fraction based on slopes of first cycle")
            slopes = []
            for ind in range(4):
                subsection = dc_amp_vec_full[
                    ind * parm_dict["VS_steps_per_full_cycle"] // 4 : (ind + 1)
                    * parm_dict["VS_steps_per_full_cycle"]
                    // 4
                ]
                slopes.append(np.mean(np.diff(subsection)))
            if verbose:
                logger.info("\t\t\tslopes for quarters: {}".format(slopes))
            frac, phas = infer_bipolar_triangular_fraction_phase(slopes)
            if verbose:
                logger.info("\t\t\tCycle fraction: {}, Phase: {}".format(frac, phas))

            for str_val, num_val in zip(["full", "1/2", "1/4", "3/4"], [1.0, 0.5, 0.25, 0.75]):
                if frac == num_val:
                    parm_dict["VS_cycle_fraction"] = str_val
                    break

            for str_val, num_val in zip(["1/4", "1/2", "3/4"], [0.25, 0.5, 0.75]):
                if phas == num_val:
                    parm_dict["VS_cycle_phase_shift"] = str_val
                    break
            if verbose:
                logger.info(
                    "\t\t\tCycle fraction: {}, Phase: {}".format(
                        parm_dict["VS_cycle_fraction"], parm_dict["VS_cycle_phase_shift"]
                    )
                )
    ...
    def _read_parms_mat(file_path, is_beps):
        ...
        BE_bin_ind = (
            np.squeeze(matread["BE_bin_ind"]) - 1
        )  # From Matlab (base 1) to Python (base 0)
    ...
    def _build_udvs_table(self, parm_dict):
        ...
        BE_signal_type = translate_val(
            parm_dict["BE_phase_content"],
            [
                "chirp-sinc hybrid",
        ...
        if BE_signal_type is None:
            raise NotImplementedError(
                'This translator does not know how to handle "BE_phase_content": "{}"'.format(
                    parm_dict["BE_phase_content"]
                )
            )
        ...
        try:
            ...
            VS_fraction = translate_val(
                parm_dict["VS_cycle_fraction"],
                ["full", "1/2", "1/4", "3/4"],
                [1.0, 0.5, 0.25, 0.75],
            )
            if VS_fraction is None:
                raise NotImplementedError(
                    'This translator does not know how to handle "VS_cycle_fraction": "{}"'.format(
                        parm_dict["VS_cycle_fraction"]
                    )
                )
        ...
        if VS_shift != 0:
            if self._verbose:
                logger.info("\tVS_shift = {}".format(VS_shift))
        ...
        VS_in_out_cond = translate_val(
            parm_dict["VS_measure_in_field_loops"],
            ["out-of-field", "in-field", "in and out-of-field"],
            [0, 1, 2],
        )
        ...
        if VS_ACDC_cond is None:
            raise NotImplementedError(
                'This translator does not know how to handle "VS_Mode": "{}"'.format(
                    parm_dict["VS_mode"]
                )
            )
        ...
        if self._verbose:
            logger.info("\t\tBuilding spectroscopy waveform")
        if VS_ACDC_cond == 0 or VS_ACDC_cond == 4:
            if self._verbose:
                logger.info("\t\t\tDC voltage spectroscopy or current mode for:")
                for key in [
                    "VS_steps",
                    "VS_fraction",
                    "VS_shift",
                    ...
                    logger.info("\t" * 4 + "{} = {}".format(key, repr(eval(key))))

            vs_amp_vec = generate_bipolar_triangular_waveform(
                VS_steps,
            ...
            if self._verbose:
                logger.info(
                    "\t\t\tgenerated triangular waveform of size: {}".format(len(vs_amp_vec))
                )

        elif VS_ACDC_cond == 2:
            if self._verbose:
                logger.info("\t\t\tAC voltage spectroscopy with time reversal")
            # Temporarily scale up the number of points in a cycle
            actual_cycle_pts = int(VS_steps // VS_fraction)
            vs_amp_vec = np.linspace(
                VS_amp / actual_cycle_pts, VS_amp, num=actual_cycle_pts, endpoint=True
            )
        ...
        if FORC_cycles > 1:
            if self._verbose:
                logger.info("\t\t\tWorking on adding FORC")
        ...
        # Build UDVS table:
        if self._verbose:
            logger.info("\t\tBuilding UDVS table")
        if VS_ACDC_cond == 0 or VS_ACDC_cond == 4:
            if VS_ACDC_cond == 0:
                if self._verbose:
                    logger.info("\t" * 3 + "DC Spectroscopy mode. Appending zeros")
                UD_dc_vec = np.vstack((vs_amp_vec, np.zeros(len(vs_amp_vec))))
            if VS_ACDC_cond == 4:
                if self._verbose:
                    logger.info("\t" * 3 + "Current mode. Tiling vs amp vec")
            ...
            if self._verbose:
                logger.info("\t\t\tAC voltage spectroscopy")
    ...
    @staticmethod
    def _get_random_spectra(
        parsers, num_pixels, num_udvs_steps, num_bins, num_spectra=100, verbose=False
    ):
        """
        Parameters
        ----------
        ...
        """
        if verbose:
            logger.info("\t" * 4 + "Getting random spectra for Q factor testing")
            logger.info(
                "\t" * 4
                + "num_pixels: {} num_udvs_steps: {}, num_bins: {}, num_spectra: {}".format(
                    num_pixels, num_udvs_steps, num_bins, num_spectra
                )
            )
        ...
        if verbose and False:
            logger.info("\t" * 4 + "Selecting the following random pixels, UDVS steps, parsers")
            logger.info("\t" * 4 + "num_spectra: {}".format(num_spectra))
            logger.info("\t" * 4 + "selected_pixels:\n{}".format(selected_pixels))
            logger.info("\t" * 4 + "selected_steps:\n{}".format(selected_steps))
            logger.info("\t" * 4 + "selected_parsers:\n{}".format(selected_parsers))
        ...
        for spectra_index in range(num_spectra):
            ...
            if verbose and False:
                logger.info(
                    "\t" * 5
                    + "Seeking and reading pixel #{}".format(selected_pixels[spectra_index])
                )
            ...
            if verbose and False:
                logger.info(
                    "\t" * 5
                    + "reshaped raw vector for pixel of shape {} by"
                    " UDVS step to: {} and taking spectrum at "
                    "index: {}".format(
                        raw_vec.shape, spectrogram.shape, selected_steps[spectra_index]
                    )
                )
        ...
        if verbose:
            logger.info("\t" * 5 + "chosen spectra of shape: {}".format(chosen_spectra.shape))
