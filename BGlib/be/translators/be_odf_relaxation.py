...
class BEodfRelaxationTranslator(Translator):
    ...
    def translate(self, file_path, show_plots=True, save_plots=True, do_histogram=False):
        ...
        (bin_inds, bin_freqs, bin_FFT, ex_wfm, dc_amp_vec) = self.__readOldMatBEvecs(
            path_dict["old_mat_parms"]
        )
        """
        ...
        h5_UDVS = chan_grp.create_dataset("UDVS", data=UDVS_mat, dtype=float)
        write_simple_attrs(h5_UDVS, {"labels": UDVS_labs, "units": UDVS_units})

        h5_bin_steps = chan_grp.create_dataset(
            "Bin_Steps", data=np.arange(bins_per_step, dtype=np.uint32), dtype=np.uint32
        )

        # Need to add the Bin Waveform type - infer from UDVS
        exec_bin_vec = self.signal_type * np.ones(len(bin_inds), dtype=np.int32)
        h5_wfm_typ = chan_grp.create_dataset("Bin_Wfm_Type", data=exec_bin_vec, dtype=np.int32)

        h5_bin_inds = chan_grp.create_dataset("Bin_Indices", data=bin_inds, dtype=np.uint32)
        h5_bin_freq = chan_grp.create_dataset("Bin_Frequencies", data=bin_freqs, dtype=float)
        h5_bin_FFT = chan_grp.create_dataset("Bin_FFT", data=bin_FFT, dtype=np.complex64)
        ...
        spec_vals, spec_inds, spec_vals_labs, spec_vals_units, spec_vals_names = createSpecVals(
            UDVS_mat, spec_inds, bin_freqs, exec_bin_vec, parm_dict, UDVS_labs, UDVS_units
        )

        spec_dims = list()
        for row_ind, row_name in enumerate(spec_vals_labs):
            spec_dims.append(Dimension(row_name, spec_vals_units[row_ind], spec_vals[row_ind]))

        pixel_chunking = maxReadPixels(
            10240, num_pix * num_actual_udvs_steps, bins_per_step, np.dtype("complex64").itemsize
        )
        chunking = np.floor(np.sqrt(pixel_chunking))
        chunking = max(1, chunking)
        chunking = min(num_actual_udvs_steps, num_pix, chunking)
        self.h5_main = write_main_dataset(
            chan_grp,
            (num_pix, tot_bins),
            "Raw_Data",
            "Piezoresponse",
    ...
    def _read_data(self, real_path, imag_path, parm_dict):
        ...
        for pix_ind in range(num_pix):
            logger.info(
                "Reading pixel #%s, file position %s", pix_ind, hex(pix_ind * bytes_per_pix)
            )
            pix_vec = np.fromstring(
                f_real.read(int(bytes_per_pix)), dtype="f"
    ...
    def __getParmsFromOldMat(file_path):
        ...
        if position_vec[0] != position_vec[1] or position_vec[2] != position_vec[3]:
            ...
            parm_dict["grid_num_rows"] = int(
                position_vec[0]
            )  # set to number of present cols and rows
        ...
        if VS_parms[0] == 0:
            ...
            parm_dict["VS_amplitude_[V]"] = 0.5 * (
                max(dc_amp_vec_full) - min(dc_amp_vec_full)
            ...
            parm_dict["VS_offset_[V]"] = (
                0  # this is not correct. Fix manually when it comes to UDVS generation?
            )
        else:
    ...
    def __buildUDVSTable(self, parm_dict):
        ...
        if VS_ACDC_cond == 0 or VS_ACDC_cond == 4:  # DC voltage spectroscopy or current mode
            ...
            VS_amp_vec = VS_amp * (
                np.hstack((VS_amp_vec_1, VS_amp_vec_2, VS_amp_vec_3, VS_amp_vec_4))
            )
            VS_amp_vec = np.roll(
                VS_amp_vec, int(np.floor(VS_steps / VS_fraction * VS_shift))
        ...
        if VS_ACDC_cond == 0 or VS_ACDC_cond == 4:  # relaxation measurements
            ...
            for step_num in np.arange(0, VS_steps):
                step_values = np.arange(
                    int(step_num) * int(BE_repeats) * 2, (int(step_num) + 1) * int(BE_repeats) * 2
                )
