...
from sidpy.hdf.dtype_utils import stack_real_to_compound, flatten_compound_to_real
...
"""
loop_metrics32 = np.dtype(
    {
        "names": ["Area", "Centroid x", "Centroid y", "Rotation Angle [rad]", "Offset"],
...
crit32 = np.dtype(
    {
        "names": ["AIC_loop", "BIC_loop", "AIC_line", "BIC_line"],
        "formats": [float, float, float, float],
    }
)

__field_names = ["a_0", "a_1", "a_2", "a_3", "a_4", "b_0", "b_1", "b_2", "b_3", "R2 Criterion"]
loop_fit32 = np.dtype({"names": __field_names, "formats": [float for name in __field_names]})
...
class BELoopFitter(Fitter):
    ...
    def __init__(self, h5_main, be_data_type, vs_mode, vs_cycle_frac, **kwargs):
        """
        ...
        super(BELoopFitter, self).__init__(h5_main, "Loop_Fit", variables=None, **kwargs)
        ...
        if "DC_Offset" in self.h5_main.spec_dim_labels:
            ...
            raise ValueError(
                'Neither "DC_Offset", nor "write_bias" were '
    ...
    def _check_validity(h5_main, data_type, vs_mode, vs_cycle_frac):
        ...
        if data_type.lower() == "bepsdata" or data_type.lower() == "beps2banddata":
            ...
            if vs_mode != "cKPFM":
                raise ValueError('Provided dataset has an unsupported VS_mode: "' + vs_mode + '"')
        else:
            raise NotImplementedError(
                "Loop fitting not supported for Band Excitation experiment type: {}".format(
                    data_type
                )
            )
    ...
    def _create_projection_datasets(self):
        ...
        _fit_spec_index = self.h5_main.spec_dim_labels.index(self._fit_dim_name)
        ...
        cycle_start_inds = np.argwhere(self.h5_main.h5_spec_inds[_fit_spec_index, :] == 0).flatten()
        tot_cycles = cycle_start_inds.size
        if self.verbose and self.mpi_rank == 0:
            logger.info(
                "Found {} cycles starting at indices: {}".format(tot_cycles, cycle_start_inds)
            )
        ...
        self.h5_projected_loops = create_empty_dataset(
            self.h5_main, float, "Projected_Loops", h5_group=self.h5_results_grp
        )
        ...
        self.h5_loop_metrics = write_main_dataset(
            self.h5_results_grp,
            (self.h5_main.shape[0], tot_cycles),
            "Loop_Metrics",
            "Metrics",
            "compound",
            None,
    ...
    def _create_guess_datasets(self):
        ...
        self._h5_guess = create_empty_dataset(self.h5_loop_metrics, loop_fit32, "Guess")
    ...
    def _read_data_chunk(self):
        ...
        if self.verbose and self.mpi_rank == 0:
            logger.info(
                "BELoopFitter got data chunk of shape {} from Fitter.".format(self.data.shape)
            )
        ...
        self._num_forcs = int(
            any([targ in self.h5_main.spec_dim_labels for targ in ["FORC", "FORC_Cycle"]])
        )
        ...
        for forc_ind in range(num_reps):
            ...
            if self._num_forcs:
                this_forc_spec_inds = np.where(self.h5_main.h5_spec_inds[forc_pos] == forc_ind)[0]
            else:
                this_forc_spec_inds = np.ones(shape=self.h5_main.h5_spec_inds.shape[1], dtype=bool)

            if self._num_forcs:
                this_forc_dc_vec = get_unit_values(
                    self.h5_main.h5_spec_inds[all_but_forc_rows][:, this_forc_spec_inds],
                    self.h5_main.h5_spec_vals[all_but_forc_rows][:, this_forc_spec_inds],
                    all_dim_names=list(np.array(self.h5_main.spec_dim_labels)[all_but_forc_rows]),
            ...
            this_forc_nd, success = reshape_to_n_dims(
                this_forc_2d, h5_pos=None, h5_spec=self.h5_main.h5_spec_inds[:, this_forc_spec_inds]
            )
            ...
            if self.verbose and self.mpi_rank == 0:
                logger.info("After reshaping to N dimensions: shape: {}".format(this_forc_nd.shape))
                logger.info(
                    "Will reorder to slow-to-fast as: {} and squeeze FORC out".format(order_to_s2f)
                )

            this_forc_nd_s2f = this_forc_nd.transpose(order_to_s2f).squeeze()  # squeeze out FORC
            ...
            if self.verbose and self.mpi_rank == 0:
                logger.info(
                    "Reordered to S2F: {}, {}".format(this_forc_nd_s2f.shape, dim_names_s2f)
                )
            ...
            if self.verbose and self.mpi_rank == 0:
                logger.info("Transpose for reordering to rest, DC: {}".format(rest_dc_order))
            ...
            if self.verbose and self.mpi_rank == 0:
                logger.info("After reodering: {}, {}".format(rest_dc_nd.shape, rest_dc_names))

            dc_rest_2d = rest_dc_nd.reshape(
                np.prod(rest_dc_nd.shape[:-1]), np.prod(rest_dc_nd.shape[-1])
            )

            if self.verbose and self.mpi_rank == 0:
                logger.info("Shape after flattening to 2D: {}".format(dc_rest_2d.shape))
    ...
    def _read_guess_chunk(self):
        ...
        if self.verbose and self.mpi_rank == 0:
            logger.info(
                "_read_guess_chunk got guess of shape: {} from super.".format(self._guess.shape)
            )
        ...
        num_forcs = int(
            any([targ in self._h5_guess.spec_dim_labels for targ in ["FORC", "FORC_Cycle"]])
        )
        if num_forcs:
        ...
        for forc_ind in range(num_reps):
            if self.verbose and self.mpi_rank == 0:
                logger.info("\nWorking on FORC #{}".format(forc_ind))
            if num_forcs:
                this_forc_spec_inds = np.where(self._h5_guess.h5_spec_inds[forc_pos] == forc_ind)[0]
            else:
            ...
            this_forc_nd, success = reshape_to_n_dims(
                this_forc_2d,
                h5_pos=None,
                h5_spec=self._h5_guess.h5_spec_inds[:, this_forc_spec_inds],
            )
            ...
            float_mat = np.zeros(
                shape=list(dc_rest_2d.shape) + [len(loop_fit32.names) - 1], dtype=float
            )
            if self.verbose and self.mpi_rank == 0:
                logger.info("Created empty float matrix of shape: {}.".format(float_mat.shape))
            for ind, field_name in enumerate(loop_fit32.names[:-1]):
                float_mat[..., ind] = dc_rest_2d[field_name]

            if self.verbose and self.mpi_rank == 0:
                logger.info("Shape after flattening to float: {}.".format(float_mat.shape))
    ...
    def _project_loop(sho_response, dc_offset):
        ...
        pix_dict = projectLoop(
            np.squeeze(dc_offset), sho_response["Amplitude [V]"], sho_response["Phase [rad]"]
        )
    ...
    @staticmethod
    def __compute_batches(data_mat_list, ref_vec_list, map_func, req_cores, verbose=False):
        """
        ...
        if verbose:
            logger.info(
                "Rank {} starting computation on {} cores (requested {} cores)".format(
        ...
        if cores > 1:
            ...
            for loops_2d, curr_vdc in zip(data_mat_list, ref_vec_list):
                values += [joblib.delayed(map_func)(x, [curr_vdc]) for x in loops_2d]
            ...
            results = []
            for loops_2d, curr_vdc in zip(data_mat_list, ref_vec_list):
                results += [map_func(vector, curr_vdc) for vector in loops_2d]
    ...
    def _unit_compute_guess(self):
        ...
        if self.verbose and self.mpi_rank == 0:
            logger.info(
                "Unit computation found {} FORC datasets with {} corresponding DC vectors".format(
                    len(resp_2d_list), len(dc_vec_list)
                )
            )
            logger.info("First dataset of shape: {}".format(resp_2d_list[0].shape))

        results = self.__compute_batches(
            resp_2d_list, dc_vec_list, self._project_loop, self._cores, verbose=self.verbose
        )
        ...
        proj_loops = np.zeros(shape=(len(results), self.data[0][0].shape[1]), dtype=float)
        ...
        # NOW do the guess:
        proj_forc = proj_loops.reshape(
            (len(dc_vec_list), len(results) // len(dc_vec_list), proj_loops.shape[-1])
        )

        if self.verbose and self.mpi_rank == 0:
            logger.info(
                "Reshaped projected loops from {} to: {}".format(proj_loops.shape, proj_forc.shape)
            )
    ...
    def set_up_guess(self, h5_partial_guess=None):
        ...
        self.parms_dict = {
            "projection_method": "pycroscopy BE loop model",
    ...
    def set_up_fit(
        ...
        # ask super to take care of the rest, which is a standardized operation
        super(BELoopFitter, self).set_up_fit(h5_partial_fit=h5_partial_fit, h5_guess=h5_guess)
    ...
    def _unit_compute_fit(self):
        ...
        for dc_vec, loops_2d, guess_parms in zip(dc_vec_list, resp_2d_list, self._guess):
            ...
            if self.verbose and self.mpi_rank == 0:
                logger.info(
                    "Computing on set: DC: {}<{}>, loops: {}<{}>, Guess: {}<{}>".format(
                        vdc_shifted.shape,
                        vdc_shifted.dtype,
                        loops_2d_shifted.shape,
                        loops_2d_shifted.dtype,
                        guess_parms.shape,
                        guess_parms.dtype,
                    )
                )
            ...
            for loop_resp, loop_guess in zip(loops_2d_shifted, guess_parms):
                curr_results = opt_func(
                    obj_func, loop_guess, args=[loop_resp, vdc_shifted], **solver_options
                )
        ...
        if self.mpi_size == 1:
            ...
            if self.verbose and self.mpi_rank == 0:
                logger.info(
                    "Dask Setup time: {} sec. Compute time: {} sec".format(t1 - t0, t2 - t1)
                )
        else:
            if self.verbose:
    ...
    def extract_loop_parameters(h5_loop_fit, nuc_threshold=0.03):
        ...
        h5_loop_parameters = create_empty_dataset(
            h5_loop_fit,
            dtype=switching32,
            dset_name=dset_name,
    ...
    def _unit_compute_fit_jl_broken(self):
        ...
        if self.verbose and self.mpi_rank == 0:
            logger.info(
                "_unit_compute_fit got:\nobj_func: {}\nsolver_options: {}".format(
                    obj_func, solver_options
                )
            )
        ...
        if self.mpi_size > 1:
            if self.verbose:
                logger.info("Rank {}: About to start serial computation.".format(self.mpi_rank))

            self._results = list()
            for dc_vec, loops_2d, guess_parms in zip(dc_vec_list, resp_2d_list, self._guess):
                if self.verbose:
                    logger.info(
                        "Setting up delayed joblib based on DC: {}<{}>, loops: {}<{}>, Guess: {}<{}>".format(
                            dc_vec.shape,
                            dc_vec.dtype,
                            loops_2d.shape,
                            loops_2d.dtype,
                ...
                for loop_resp, loop_guess in zip(loops_2d_shifted, guess_parms):
                    curr_results = least_squares(
                        obj_func, loop_guess, args=[loop_resp, vdc_shifted], **solver_options
                    )
                    self._results.append(curr_results)
        else:
            cores = recommend_cpu_cores(
                len(resp_2d_list) * resp_2d_list[0].shape[0], verbose=self.verbose
            )
            if self.verbose:
            ...
            for dc_vec, loops_2d, guess_parms in zip(dc_vec_list, resp_2d_list, self._guess):
                if self.verbose:
                    logger.info(
                        "Setting up delayed joblib based on DC: {} loops: {}, Guess: {}".format(
                            dc_vec.shape, loops_2d.shape, guess_parms.shape
                        )
                    )
                """
                Shift the loops and vdc vector
        ...
        if self.verbose and self.mpi_rank == 0:
            logger.info("Finished computing fits on {} objects".format(len(self._results)))
    ...
    def _reformat_results_chunk(
        ...
        if verbose:
            logger.info(
                "Dimension sizes & order: {} and names: {} that flattened "
        ...
        map_to_s2f = [first_n_dim_names.index(dim_name) for dim_name in dim_labels_s2f]
        if verbose:
            logger.info(
                "Will permute as: {} to arrange dimensions from slowest to fastest varying".format(
                    map_to_s2f
                )
            )
        ...
        if verbose:
            logger.info(
                "Shape: {} and dimension labels: {} of results arranged from"
        ...
        if verbose:
            logger.info("Results will be flattend to: {}.".format((pos_size, spec_size)))
    ...
    def _write_guess_chunk(self):
        ...
        loops_2d = self._reformat_results_chunk(
            self._num_forcs,
            proj_loops,
        ...
        mets_2d = self._reformat_results_chunk(
            self._num_forcs,
            loop_mets,
        ...
        guess_2d = self._reformat_results_chunk(
            self._num_forcs,
            all_guesses,
            self._pre_flattening_shape[:-1],
            self._pre_flattening_dim_name_order[:-1],
        ...
        if self.verbose and self.mpi_rank == 0:
            ...
            logger.info(
                "Writing Guesses of shape: {} and data type: {} to a dataset of shape: {} and data type {}".format(
                    guess_2d.shape, guess_2d.dtype, self._h5_guess.shape, self._h5_guess.dtype
                )
            )
    ...
    def _write_fit_chunk(self):
        ...
        temp = np.array([np.hstack([result.x, result.fun]) for result in self._results])
        ...
        fits_2d = self._reformat_results_chunk(
            self._num_forcs,
            all_fits,
            self._pre_flattening_shape[:-1],
            self._pre_flattening_dim_name_order[:-1],
        ...
        if self.verbose and self.mpi_rank == 0:
            logger.info(
                "Writing Fits of shape: {} and data type: {} to a dataset of shape: {} and data type {}".format(
                    fits_2d.shape, fits_2d.dtype, self._h5_fit.shape, self._h5_fit.dtype
                )
            )
...
def guess_loops_hierarchically(vdc_vec, projected_loops_2d):
    ...
    def _loop_fit_tree(tree, guess_mat, fit_results, vdc_shifted, shift_ind):
        ...
        curr_fit_results = fit_loop(
            vdc_shifted, np.roll(tree.value, shift_ind), guess_mat[tree.name]
        )
        ...
        for child in tree.children:
            ...
            # Fit this child:
            guess_mat, fit_mat = _loop_fit_tree(
                child, guess_mat, fit_results, vdc_shifted, shift_ind
            )
        return guess_mat, fit_results

    num_clusters = max(
        2, int(projected_loops_2d.shape[0] ** 0.5)
    )  # change this to 0.6 if necessary
    ...
    cluster_tree = ClusterTree(
        linkage_pairing[:, :2], labels, distances=linkage_pairing[:, 2], centroids=centroids
    )
    ...
    loop_guess_mat, loop_fit_results = _loop_fit_tree(
        cluster_tree.tree, loop_guess_mat, loop_fit_results, vdc_shifted, shift_ind
    )
