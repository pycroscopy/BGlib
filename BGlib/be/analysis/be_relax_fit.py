...
def fit_double_exp(x, y):
    ...
    popt = differential_evolution(
        cost_func_double_exp,
        bounds=([-100, 100], [-100, 100], [-200, 200], [-100, 100], [-200, 200]),
    ).x
    return popt
...
def fit_str_exp(x, y):
    popt, _ = curve_fit(
        str_exp, x, y, maxfev=25000, bounds=([-np.inf, 0, -np.inf], [np.inf, 1, np.inf])
    )
    return popt
...
class BERelaxFit(usid.Process):
    def __init__(
        ...
        **Currently, the BE software does not consistently encode whether spectra start with a read or write step
        ...
        self.raw_amp_reshape = self.raw_amp.reshape(
            self.h5_main_usid.pos_dim_sizes[0],
            self.h5_main_usid.pos_dim_sizes[1],
            h5_main.parent.parent.parent.attrs["num_steps"],
            -1,
        )
        self.raw_phase_reshape = self.raw_phase.reshape(
            self.h5_main_usid.pos_dim_sizes[0],
            self.h5_main_usid.pos_dim_sizes[1],
            h5_main.parent.parent.parent.attrs["num_steps"],
            -1,
        ...
        self.time_elapsed_per_step = self.h5_main.parent.parent.parent.attrs[
            "BE_pulse_duration_[s]"
        ]
        self.time_elapsed_per_spectrum = (self.no_read_steps) * self.time_elapsed_per_step
        self.all_dc_offset_values = self.h5_main.h5_spec_vals[
            1, np.argwhere(self.h5_main.h5_spec_inds[0] == 0)
        ]
        ...
        self.all_inds_split = np.array_split(
            np.arange(0, self.no_time_steps, step=1), self.no_rs_spectra
        )
        self.write_spectra = []
        ...
        if self.starts_with == "read":
            for i in range(self.no_rs_spectra):
                ...
                self.write_dc_offset_values = self.h5_main.h5_spec_vals[
                    1, np.argwhere(self.h5_main.h5_spec_vals[0] == self.no_read_steps)
                ]
                # if there is only one RS spectrum
                if isinstance(self.write_dc_offset_values, float):
                    self.write_dc_offset_values = [self.write_dc_offset_values]
        ...
        self.write_inds_split = np.split(
            np.setxor1d(self.all_inds_split, self.read_inds_split), self.no_rs_spectra
        )
    ...
    def test(self, pixel_ind):
        ...
        for j in range(self.no_read_offset):
            mixed_signal.append(
                amplitude_to_reshape[self.read_inds_split[j]]
            ...
            for i in range(self.h5_main.shape[0]):
                for j in range(self.no_read_offset):
                    ...
                    mixed_signal = np.array(
                        [
                            amp * self.sensitivity * np.cos(phase + self.phase_offset)
    ...
    def _create_results_datasets(self):
        ...
        usid.hdf_utils.write_simple_attrs(
            self.h5_results_grp, {"last_pixel": 0, "algorithm": str(self.fit_method)}
        )
        ...
        berelaxfit32 = np.dtype({"names": field_names, "formats": [float for name in field_names]})
        self.h5_results = usid.hdf_utils.write_main_dataset(
            self.h5_results_grp,
            results_shape,
            results_dset_name,
            results_quantity,
    ...
    def _write_results_chunk(self):
        ...
        results = usid.io.dtype_utils.stack_real_to_compound(
            self._results, compound_type=berelaxfit32
        )
