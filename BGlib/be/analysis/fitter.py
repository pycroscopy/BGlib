...
class Fitter(Process):
        ...
        if variables is not None:
            ...
            if not all([dim in self.h5_main.spec_dim_labels for dim in variables]):
                raise ValueError(
                    ...
                    "to be fitted: {}"
                    "".format(self.h5_main.spec_dim_labels, variables)
                )
    ...
    def _write_results_chunk(self):
        ...
        if self.verbose and self.mpi_rank == 0:
            print(
                "Writing data of shape: {} and dtype: {} to position range: "
                "{} in HDF5 dataset:{}".format(
                    source_dset.shape,
                    source_dset.dtype,
    ...
    def _create_guess_datasets(self):
        ...
        raise NotImplementedError(
            "Please override the _create_guess_datasets specific to your model"
        )
    ...
    def do_guess(self, *args, override=False, **kwargs):
        ...
        if not self.__set_up_called:
            raise ValueError("Please call set_up_guess() before calling do_guess()")
    ...
    def set_up_guess(self, h5_partial_guess=None):
        ...
        if h5_partial_guess is not None:
            raise NotImplementedError(
                "Provided h5_partial_guess cannot be used yet. Ask developer to implement"
            )
    ...
    def set_up_fit(self, h5_partial_fit=None, h5_guess=None):
        ...
        if h5_partial_fit is not None or h5_guess is not None:
            raise NotImplementedError(
                "Provided h5_partial_fit cannot be used yet. Ask developer to implement"
            )
    ...
    def _unit_compute_fit(self, obj_func, obj_func_args=[], solver_options={"jac": "cs"}):
        """
        ...
        if self.mpi_size > 1:
            if self.verbose:
                print("Rank {}: About to start serial computation.".format(self.mpi_rank))

            self._results = list()
            for pulse_resp, pulse_guess in zip(self.data, self._guess):
                curr_results = least_squares(
                    obj_func, pulse_guess, args=[pulse_resp] + obj_func_args, **solver_options
                )
        ...
        if self.verbose and self.mpi_rank == 0:
            print(
                "Finished computing fits on {} objects. Results of length: {}.".format(
                    self.data.shape[0], len(self._results)
                )
            )
