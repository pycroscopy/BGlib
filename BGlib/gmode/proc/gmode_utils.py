...
from pyUSID.io.hdf_utils import check_if_main, write_main_dataset, create_results_group
...
def test_filter(
    ...
    if show_plots:
        ...
        if frequency_filters is not None:
            ax_raw.semilogy(
                w_vec[x_lims],
                (composite_filter[x_lims] + np.min(amp)) * (np.max(amp) - np.min(amp)),
                linewidth=3,
                color="orange",
    ...
    if noise_threshold is not None:
        fft_pix_data[np.abs(fft_pix_data) < noise_floor] = (
            1e-16  # DON'T use 0 here. ipython kernel dies
        )
...
def reshape_from_lines_to_pixels(h5_main, pts_per_cycle, scan_step_x_m=None):
    ...
    spec_dims = Dimension(
        ...
        get_attr(h5_main.h5_spec_vals, "units")[0],
        single_ao,
    )
