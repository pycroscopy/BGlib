...
def do_bayesian_inference(
    ...
    for j in range(num_volt_points):
        ...
        A[j, ix - 1] = bias[j] * (1.0 - (bias[j] - x[ix - 1]) / (x[ix] - x[ix - 1]))

    A[:, num_x_steps] = dv + r_extra * bias

    # generate simulated observations
    Lapt = (
        (-1.0 * np.diag((t[:-1]) ** 0, -1) - np.diag(t[:-1] ** 0, 1) + 2.0 * np.diag(t**0, 0))
        / dt
        / dt
    )
    Lapt[0, 0] = 1.0 / dt / dt
    Lapt[-1, -1] = 1.0 / dt / dt
    observation_cov = (1.0 / gam**2) * np.eye(num_volt_points)
    # noise_term = np.linalg.lstsq(sqrtm(observation_cov),np.random.randn(N,1))[0]
    # y = IV_point
    #  Itrue + noise_term.ravel()

    Lap = (
        (-1.0 * np.diag((x[:-1]) ** 0, -1) - np.diag(x[:-1] ** 0, 1) + 2.0 * np.diag(x**0, 0))
        / dx
        / dx
    )
    Lap[0, 0] = 1.0 / dx / dx
    ...
    m0 = 3.0 * np.ones((num_x_steps, 1))
    m0 = np.append(m0, 0)

    P0 = np.zeros(shape=(num_x_steps + 1, num_x_steps + 1))
    P0[:num_x_steps, :num_x_steps] = (
        1.0 / sigma**2 * (1.0 * np.eye(num_x_steps) + np.linalg.matrix_power(Lap, 3))
    )
    P0[num_x_steps, num_x_steps] = 1.0 / sigmaC**2
    ...
    SI = np.tile(m[:num_x_steps], (num_samples, 1)).T + np.dot(
        sqrtm(Sigma[:num_x_steps, :num_x_steps]), np.random.randn(num_x_steps, num_samples)
    )
    # approximate mean and covariance of R
    mR = 1.0 / num_samples * np.sum(1.0 / SI, 1)
    m2R = 1.0 / num_samples * np.dot(1.0 / SI, (1.0 / SI).T)
    ...
    if show_plots:
        ...
        dens_cx = (
            1.0
            / np.sqrt(Sigma[num_x_steps, num_x_steps] * 2 * np.pi)
            * np.exp(-((cx - m[num_x_steps]) ** 2) / 2 / Sigma[num_x_steps, num_x_steps])
        )
        plt.plot(cx, dens_cx)
        plt.ylabel("p(C)")
        plt.xlabel("C")
...
def bayesian_inference_on_period(
    i_meas, excit_wfm, ex_freq, r_extra=110, num_x_steps=500, show_plots=False, r_max=None, **kwargs
):
    """
    ...
    rev_results = do_bayesian_inference(
        y_val[: int(0.5 * num_v_steps)] * -1,
        cos_omega_t[: int(0.5 * num_v_steps)] * -1,
        ex_freq,
        num_x_steps=half_x_steps,
        econ=True,
        show_plots=False,
        r_extra=r_extra,
        **kwargs,
    )
    forw_results = do_bayesian_inference(
        y_val[int(0.5 * num_v_steps) :],
        cos_omega_t[int(0.5 * num_v_steps) :],
        ex_freq,
        num_x_steps=half_x_steps,
        econ=True,
        show_plots=False,
    ...
    if show_plots:
        ...
        axis.plot(
            excit_wfm, i_corr_sine, color="k", label="Sine corr"
        ...
        axis.plot(
            forw_results["x"],
            forw_results["x"] / forw_results["mR"],
            color="blue",
            label="Bayes corr F",
        )
        axis.plot(
            rev_results["x"],
            rev_results["x"] / rev_results["mR"],
            color="red",
            label="Bayes corr R",
        )
        ...
        for axis, res_vec, variance_vec, name, direction in zip(
            axes.flat,
...
def plot_bayesian_spot_from_h5(h5_bayesian_grp, h5_resh, pix_ind, **kwargs):
    ...
    orig_bias = np.squeeze(
        get_auxiliary_datasets(h5_resh, aux_dset_name=["Spectroscopic_Values"])[0]
    )
...
def plot_bayesian_results(
    ...
    **kwargs,
    ...
    for type_ind, axis, pts_list, cols_set, sym_set, set_name in zip(
        ...
        axis.set_title(
            "$R(V)$ " + set_name + " at Row = " + str(pix_pos[1]) + " Col =" + str(pix_pos[0]),
        ...
        if not single_plot:
            ...
            for ind in range(1, jump_inds.size):
                ...
                if res_scatter:
                    axis.scatter(
                        bias_triang[cur_range], resistance[cur_range], color=cols_set[0], s=30
                    )
                else:
                    axis.plot(
                        bias_triang[cur_range],
                ...
                if ind == 1:
                    axis.legend(
                        ["R(V)", "R(V)+-$\sigma$"], loc="upper center", fontsize=font_size_1
                    )
        else:
            if res_scatter:
                axis.scatter(bias_triang[pts_list], resistance[pts_list], color=cols_set[0], s=30)
            else:
                axis.plot(
                    bias_triang[pts_list],
                    resistance[pts_list],
                    cols_set[0],
                    linestyle=sym_set[0],
                    linewidth=3,
    ...
    axes[2].set_title(
        "$I(V)$ at row " + str(pix_pos[0]) + ", col " + str(pix_pos[1]), fontsize=font_size_2
    )
