...
class GVSTranslator(Translator):
    ...
    def translate(self, parm_path):
        ...
        w_vec = np.arange(-0.5 * samp_rate, 0.5 * samp_rate, np.array(samp_rate / num_bins)).astype(
            float
        )
        ...
        pos_dims = [Dimension("X", "nm", num_rows), Dimension("Y", "nm", num_cols)]
        spec_dims = [
        ...
        for row_ind in range(1, num_rows + 1):
            for col_ind in range(1, num_cols + 1):
                file_path = path.join(
                    folder_path, "fSweep_r" + str(row_ind) + "_c" + str(col_ind) + ".mat"
                )
