...
from pyUSID.io.write_utils import (
    INDICES_DTYPE,
    VALUES_DTYPE,
    Dimension,
...
def build_reduced_spec_dsets(
    h5_parent_group, h5_spec_inds, h5_spec_vals, keep_dim, step_starts, basename="Spectroscopic"
):
    """
    ...
    """
    warn(
        "build_reduced_spec_dsets is available only for legacy purposes and will be REMOVED in a future release.\n"
        "Please consider using write_reduced_spec_dsets instead",
        DeprecationWarning,
    )
    ...
    for sub_name in ["_Indices", "_Values"]:
        if basename + sub_name in h5_parent_group.keys():
            raise KeyError(
                "Dataset: {} already exists in provided group: {}".format(
                    basename + sub_name, h5_parent_group.name
                )
            )
