...
class GIVTranslator(Translator):
    ...
    def translate(self, parm_path):
        ...
        with h5py.File(h5_path) as h5_f:
            ...
            pos_dims = Dimension(
                "Y",
                "m",
                np.linspace(0, parm_dict["grid_scan_height_[m]"], parm_dict["grid_num_rows"]),
            )
            ...
            for chan_index in range(num_ai_chans):
                ...
                h5_raw = write_main_dataset(
                    h5_chan_grp,
                    (parm_dict["grid_num_rows"], excit_wfm.size),
                    "Raw_Data",
                    "Current",
    ...
    def _read_data(self, parm_dict, folder_path):
        ...
        if parm_dict["excitation_extra_pts"] == 0:
            ...
            main_data = slice(
                parm_dict["excitation_pulse_points"], -1 * parm_dict["excitation_extra_pts"]
            )

        for line_ind in range(parm_dict["grid_num_rows"]):
            if line_ind % np.round(parm_dict["grid_num_rows"] / 10) == 0:
                logger.info(
                    "Reading data in line %s of %s", line_ind + 1, parm_dict["grid_num_rows"]
                )
            file_path = path.join(folder_path, "line_" + str(line_ind + 1) + ".mat")
            if path.exists(file_path):
                with h5py.File(file_path, "r") as h5_f:
                    h5_data = h5_f["data"]
                    if h5_data.shape[0] >= parm_dict["excitation_length"] and h5_data.shape[
                        1
                    ] == len(self.raw_datasets):
    ...
    def _read_parms(parm_path):
        ...
        with h5py.File(parm_path, "r") as h5_f:
            ...
            try:
                parm_dict["excitation_amplitude_[V]"] = np.array(h5_f["amplitude"][0][0]).astype(
                    float
                )
                parm_dict["excitation_offset_[V]"] = np.array(h5_f["offset"][0][0]).astype(float)
            except KeyError:
                parm_dict["excitation_offset_[V]"] = excit_wfm[
                    0
                ]  # need a better way to handle this.
                parm_dict["excitation_amplitude_[V]"] = np.max(
                    excit_wfm[:]
                )  # risky for non sine waves

            # Make sure to truncate the data to the point when the
            pts_per_cycle = int(
                np.round(
                    1.0 * parm_dict["IO_samp_rate_[Hz]"] / parm_dict["excitation_frequency_[Hz]"]
                )
            )
